...
*/
  ...
  extension (b: Bpmn)
    ...
    def printObjects(): Print =
      poo(
        b.ident.toString,
        pl(s"""val _bpmn = bpmn("${b.ident}")"""),
        po(
          b.processes.print()
        ),
         b.processes.printObjects()
      )
  end extension
  ...
  extension (process: Process[?,?])
      def print(): Print =
        pl(s"processes.${process.id}")

      def printObjects(): Print =
        pa2(
          po(
            pl(s"""val ${process.id}Ident = "${process.id}""""),
            pl(s"""val ${process.id} = process(${process.id}Ident)"""),
